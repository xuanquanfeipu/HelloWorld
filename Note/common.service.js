...
(function() {
  ...
  function CommService($http, SweetAlert) {
    ...
    this.downloadExl = function(filename, data, type) {
      ...
      wb.Sheets['Sheet1'] = ws;
      saveAs(new Blob([this.s2ab(XLSX.write(wb, wopts))], {
    ...
    /*
     * exportToExcel方法的x座标取值opt.xAxis.data
     * 人口库的opt.xAxis是一个数组
     * 先建立此方法以区分，后续看能不能合为一个
     * 10137096 Bruce huang
     * */
    this.exportToExcelForMxA = function(filename, opt, axisData) {
      var series = opt.series;
      var data = [];
      var title = ['指标'];
      for (var t in axisData) {
    ...
    var strHasCapLetter = function(pwd) {
      for (var i = 0; i < pwd.length; i++) {
        var ch = pwd.charAt(i);
    ...
    var strHasNonCapLetter = function(pwd) {
      for (var i = 0; i < pwd.length; i++) {
        var ch = pwd.charAt(i);
        if (ch <= 'z' && ch >= 'a') return true;
      }
    ...
    var checkOnlyContainsNumber = function(numberstr) {
    ...
    var strHasSpecialLetter = function(str) {
    ...
    var checkComplexPassword = function(cpwd) {
      var i = 0;
    ...
    this.showWarnInfo = function(msg) {
      SweetAlert.swal({
        title: '警告',
        text: msg,
        closeOnConfirm: true,
