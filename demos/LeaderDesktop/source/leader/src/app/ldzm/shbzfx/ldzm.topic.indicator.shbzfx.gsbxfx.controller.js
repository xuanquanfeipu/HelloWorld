...
(function () {
  ...
  angular
    .module('smartCore')
    .controller('LdzmTopicIndShbzfxGsbxfxController', LdzmTopicIndShbzfxGsbxfxController);

  /** @ngInject */
  function LdzmTopicIndShbzfxGsbxfxController($http, devUrl, CommService) {
    var vm = this;

    vm.title = "领导桌面-社会保障分析-工伤保险分析";
    ...
    loadStatData(devUrl + 'leader/socialinsurance/gsbxxx', function(data) {
      ...
      for (var i=0; i<vm.years.length; i++) {
        var year = vm.years[i];
        parseDataWithYear(data.gsbxDataList, year, vm.dataList, ['cbrs','jfrs','sr','zc','yldy','scdy','gwdy',
          'xsdyjezs','cbdws','xsdyrs','zybrs','dqjy','ljjy']);
      }
    ...
    function getOption2(yAxis) {
      var pieLegendData=['医疗待遇','伤残待遇','工亡待遇'];
      var pieData=[
        {name:'医疗待遇',value:vm.dataList.yldyList[vm.years.length-1]},
        {name:'伤残待遇',value:vm.dataList.scdyList[vm.years.length-1]},
        {name:'工亡待遇',value:vm.dataList.gwdyList[vm.years.length-1]}];
      return {
        title:
          {
            text:'享受待遇总数',
            subtext: vm.dataList.xsdyjezsList[vm.years.length-1]+'亿元',
            x:'49%',
            y:yAxis+'%',
            textAlign: "center",
            subtextStyle:{
    ...
    function renderData() {
      ...
      vm.xsdyrs = vm.dataList.xsdyrsList[vm.years.length-1];
      vm.zybrs = vm.dataList.zybrsList[vm.years.length-1];
