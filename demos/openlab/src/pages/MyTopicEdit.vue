        ...
        <div v-if="!isShow">
                ...
                <el-col :span="24" style="box-sizing: border-box;position:relative">
                    <div class="breadLink">
                        <el-breadcrumb separator-class="el-icon-arrow-right">
                            ...
                            <!-- <el-breadcrumb-item :to="{path:'/sectionDetail',query:{id:1}}">Yita大数据</el-breadcrumb-item> -->
                            <el-breadcrumb-item><span style="color: #008fd5;">编辑帖子</span></el-breadcrumb-item>
                        </el-breadcrumb>
                    </div>
                </el-col>
            </el-row>
                    ...
                    <div class="titlebody">
                        ...
                        <!-- <div class="input"> -->
                            <el-form-item prop="inputtitle" style="width:50%;float:left">
                                <el-input placeholder="请输入内容" v-model="form.inputtitle" :disabled="true">
                        ...
                        <!-- <div class="select"> -->
                                <el-form-item prop="moduleTypeId" style="float:left;margin-left:60px">
                                    <el-select v-model="form.moduleTypeId" placeholder="请选择" @change="sectionNameList" :disabled="true">
                                ...
                                <el-form-item prop="moduleId" style="float:left;margin-left:34px">
                                    <el-select v-model="form.moduleId" placeholder="请选择" :disabled="true">
            ...
            <el-row>
                <el-col :span="24" style="width:100%;position:relative;padding:0 20px 60px 20px;margin-bottom:40px;">
                    <div class="uedit" v-if="isFlag">
                        <UE :defaultMsg=defaultMsg :config=config ref="ue"></UE>
                    </div>
                    <div class="btnGroup">
                        <el-button type="primary" style="background-color:#0084FF;float:left;width:110px" @click="submit('1','form')">保存</el-button>
                        <el-button type="primary" style="background-color:#fff;float:left;margin-left:10px;width:110px;color:#0084FF;border:1px solid #0084FF;" @click="submit('0','form')">取消</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
...
export default {
  ...
  methods:{
      getTopicDetail(){
           var _this = this;
          this.$http({
              url:"topic/getEdit/"+this.topicid+'?date='+new Date().getTime(),
              method:'get',
              headers:{
              ...
              if(0 == res.data.returnCode){
                 _this.form.inputtitle = res.data.data.title;
                 _this.form.moduleTypeId = res.data.data.moduleTypeId;
                 _this.moduleid = res.data.data.moduleId;
                 _this.defaultMsg = res.data.data.body;
                 _this.isFlag = true
    ...
    //   获取版块分类下拉框列表
      sectionCategoryList(){
          var _this = this;
          this.$http({
              url:"topic/moduleType"+'?date='+new Date().getTime(),
              method:'get',
              headers:{
                'X-Access-Token': sessionStorage.getItem('accessToken'),
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
    ...
    sectionNameList(){
        ...
        this.$http({
            url:'topic/module/'+this.form.moduleTypeId+'?date='+new Date().getTime(),
            method:'get',
            headers:{
                'X-Access-Token': sessionStorage.getItem('accessToken'),
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
    ...
    submit(state,form){
        var url = this.$route.query.isURL;
        var _this = this;
        if(state==0){
            ...
            var message='保存成功';
            if(this.state != 0){
                var pathUrlParams = {path:url, query:{
                            id:this.moduleid
                        }};
            }else{
                var pathUrlParams = {path:'/peopleCenter/personalHomepage'};
            ...
            var url = 'topic/update/'+this.topicid;
            var qs = require('qs')
