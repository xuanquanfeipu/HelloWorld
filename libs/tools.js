...
xhook.before(function(request) {
	...
	//客户端获取Mac地址需要在登录请求附带header信息
	if(store('clntMAC')){
		request.headers["clntMAC"] = store('clntMAC');
	}
});
/**
 * 初始化脚本文件装载工具
...
$Boot.Config = function() {
	...
	this.getBackSpaceUrl = function(layer){
		var pathname = location.pathname;
		var member = pathname.split('/');
		var len = member.length;
		var preUrl = '';
		for(var i = 1; i < len - layer - 1; i++){
			preUrl = preUrl + '/' + member[i];
		}
		return preUrl;
	}
...
function getBackSpaceUrl(layer){
 return $Boot.bootConfig.getBackSpaceUrl(layer);
}
...
function setLogoAction(hrefAction){	
	if(hrefAction){
		var zteBrand = document.getElementsByClassName("zte-navbar-brand")[0];
		if(zteBrand){
			zteBrand.style.cursor = "pointer";
			zteBrand.href = hrefAction;
		}
	}	
}
